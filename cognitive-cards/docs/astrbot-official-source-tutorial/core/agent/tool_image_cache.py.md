# 文件教程：core/agent/tool_image_cache.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\agent\tool_image_cache.py`
- 文件类型：`.py`
- 文件大小：`5398` 字节
- 所属目录教程：[core/agent](README.md)

## 它是做什么的

Tool image cache module for storing and retrieving images returned by tools

## 角色判断

这个文件位于核心实现层，通常承载底层机制、抽象或运行时行为。

## 模块文档字符串

Tool image cache module for storing and retrieving images returned by tools.

This module allows LLM to review images before deciding whether to send them to users.

## 顶层导入

- `base64`
- `os`
- `time`
- `from dataclasses import dataclass, field`
- `from typing import ClassVar`
- `from astrbot import logger`
- `from astrbot.core.utils.astrbot_path import get_astrbot_temp_path`

## 顶层类

- `CachedImage`：Represents a cached image from a tool call
- `ToolImageCache`：Manages cached images from tool calls

## 顶层函数

- 无顶层函数定义。

## 阅读建议

- 建议从模块文档字符串、顶层类和顶层函数出发，再搜索这些名字的调用位置。

## 同目录相关文件

- [agent.py](agent.py.md)
- [handoff.py](handoff.py.md)
- [hooks.py](hooks.py.md)
- [mcp_client.py](mcp_client.py.md)
- [message.py](message.py.md)
- [response.py](response.py.md)
- [run_context.py](run_context.py.md)
- [tool.py](tool.py.md)
- [tool_executor.py](tool_executor.py.md)

## 维护提示

- 这份文件教程是基于当前 AstrBot 官方源码快照自动生成的摘要。
- 如果你准备修改当前文件，建议先同步阅读它的所属目录教程，再搜索它在全项目中的引用位置。