# 文件教程：core/astr_agent_hooks.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\astr_agent_hooks.py`
- 文件类型：`.py`
- 文件大小：`3729` 字节
...
这个文件主要定义了 MainAgentHooks、EmptyAgentHooks 等顶层类。
...
- `MainAgentHooks`：建议阅读类定义与方法名来判断职责。
- `EmptyAgentHooks`：建议阅读类定义与方法名来判断职责。
