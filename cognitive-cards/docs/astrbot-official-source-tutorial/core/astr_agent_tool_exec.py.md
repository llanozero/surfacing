# 文件教程：core/astr_agent_tool_exec.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\astr_agent_tool_exec.py`
- 文件类型：`.py`
- 文件大小：`30118` 字节
...
这个文件主要定义了 FunctionToolExecutor 等顶层类。
...
- `from astrbot.core.astr_main_agent_resources import BACKGROUND_TASK_RESULT_WOKE_SYSTEM_PROMPT`
...
- `FunctionToolExecutor`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `call_local_llm_tool`：执行本地 LLM 工具的处理函数并处理其返回结果
