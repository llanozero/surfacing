# 文件教程：core/computer/booters/boxlite.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\computer\booters\boxlite.py`
- 文件类型：`.py`
- 文件大小：`7038` 字节
...
这个文件主要定义了 MockShipyardSandboxClient、BoxliteBooter 等顶层类。
...
- `from shipyard.python import PythonComponent`
...
- `MockShipyardSandboxClient`：建议阅读类定义与方法名来判断职责。
- `BoxliteBooter`：建议阅读类定义与方法名来判断职责。
