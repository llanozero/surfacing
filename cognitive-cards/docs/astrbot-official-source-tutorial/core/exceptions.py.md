# 文件教程：core/exceptions.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\exceptions.py`
- 文件类型：`.py`
- 文件大小：`851` 字节
...
这个文件主要定义了 AstrBotError、ProviderNotFoundError、EmptyModelOutputError 等顶层类。
...
- `AstrBotError`：Base exception for all AstrBot errors
- `ProviderNotFoundError`：Raised when a specified provider is not found
- `EmptyModelOutputError`：Raised when the model response contains no usable assistant output
- `KnowledgeBaseUploadError`：Raised when knowledge base upload fails with a user-facing message
