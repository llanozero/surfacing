# 文件教程：core/platform/astrbot_message.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\platform\astrbot_message.py`
- 文件类型：`.py`
- 文件大小：`2781` 字节
...
这个文件主要定义了 MessageMember、Group、AstrBotMessage 等顶层类。
...
- `MessageMember`：建议阅读类定义与方法名来判断职责。
- `Group`：建议阅读类定义与方法名来判断职责。
- `AstrBotMessage`：AstrBot 的消息对象
