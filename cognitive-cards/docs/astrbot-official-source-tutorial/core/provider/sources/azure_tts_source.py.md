# 文件教程：core/provider/sources/azure_tts_source.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\azure_tts_source.py`
- 文件类型：`.py`
- 文件大小：`11011` 字节
...
这个文件主要定义了 OTTSProvider、AzureNativeProvider、AzureTTSProvider 等顶层类。
...
- `OTTSProvider`：建议阅读类定义与方法名来判断职责。
- `AzureNativeProvider`：建议阅读类定义与方法名来判断职责。
- `AzureTTSProvider`：建议阅读类定义与方法名来判断职责。
