# 文件教程：core/provider/sources/bailian_rerank_source.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\bailian_rerank_source.py`
- 文件类型：`.py`
- 文件大小：`8835` 字节
...
这个文件主要定义了 BailianRerankError、BailianAPIError、BailianNetworkError 等顶层类。
...
- `BailianRerankError`：百炼重排序服务异常基类
- `BailianAPIError`：百炼API返回错误
- `BailianNetworkError`：百炼网络请求错误
- `BailianRerankProvider`：阿里云百炼文本重排序适配器
