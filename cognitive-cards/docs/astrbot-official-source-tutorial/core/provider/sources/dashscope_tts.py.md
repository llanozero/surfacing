# 文件教程：core/provider/sources/dashscope_tts.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\dashscope_tts.py`
- 文件类型：`.py`
- 文件大小：`5774` 字节
...
这个文件主要定义了 ProviderDashscopeTTSAPI 等顶层类。
...
- `ProviderDashscopeTTSAPI`：建议阅读类定义与方法名来判断职责。
