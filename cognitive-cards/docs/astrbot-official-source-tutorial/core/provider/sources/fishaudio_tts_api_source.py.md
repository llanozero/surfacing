# 文件教程：core/provider/sources/fishaudio_tts_api_source.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\fishaudio_tts_api_source.py`
- 文件类型：`.py`
- 文件大小：`6409` 字节
...
这个文件主要定义了 ServeReferenceAudio、ServeTTSRequest、ProviderFishAudioTTSAPI 等顶层类。
...
- `ServeReferenceAudio`：建议阅读类定义与方法名来判断职责。
- `ServeTTSRequest`：建议阅读类定义与方法名来判断职责。
- `ProviderFishAudioTTSAPI`：建议阅读类定义与方法名来判断职责。
...
- [mimo_api_common.py](mimo_api_common.py.md)
- 其余 19 个同目录文件省略
