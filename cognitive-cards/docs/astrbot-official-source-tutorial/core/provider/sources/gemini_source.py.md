# 文件教程：core/provider/sources/gemini_source.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\gemini_source.py`
- 文件类型：`.py`
- 文件大小：`44856` 字节
...
这个文件主要定义了 SuppressNonTextPartsWarning、ProviderGoogleGenAI 等顶层类。
...
- `SuppressNonTextPartsWarning`：过滤 Gemini SDK 中的非文本部分警告
- `ProviderGoogleGenAI`：建议阅读类定义与方法名来判断职责。
