# 文件教程：core/provider/sources/mimo_api_common.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\mimo_api_common.py`
- 文件类型：`.py`
- 文件大小：`4403` 字节
...
- `from astrbot.core.utils.tencent_record_helper import convert_to_pcm_wav, tencent_silk_to_wav`

## 顶层类

- `MiMoAPIError`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `normalize_timeout`：建议阅读函数签名和调用位置来判断用途。
- `build_headers`：建议阅读函数签名和调用位置来判断用途。
- `get_temp_dir`：建议阅读函数签名和调用位置来判断用途。
- `create_http_client`：建议阅读函数签名和调用位置来判断用途。
- `build_api_url`：建议阅读函数签名和调用位置来判断用途。
- `_detect_audio_format`：建议阅读函数签名和调用位置来判断用途。
- `prepare_audio_input`：建议阅读函数签名和调用位置来判断用途。
- `cleanup_files`：建议阅读函数签名和调用位置来判断用途。
