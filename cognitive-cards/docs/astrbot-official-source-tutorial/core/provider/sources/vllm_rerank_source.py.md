# 文件教程：core/provider/sources/vllm_rerank_source.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\provider\sources\vllm_rerank_source.py`
- 文件类型：`.py`
- 文件大小：`2752` 字节
...
这个文件主要定义了 VLLMRerankProvider 等顶层类。
...
- `VLLMRerankProvider`：建议阅读类定义与方法名来判断职责。
