# 文件教程：core/star/filter/permission.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\star\filter\permission.py`
- 文件类型：`.py`
- 文件大小：`970` 字节
...
这个文件主要定义了 PermissionType、PermissionTypeFilter 等顶层类。
...
- `PermissionType`：权限类型
- `PermissionTypeFilter`：建议阅读类定义与方法名来判断职责。
