# 文件教程：core/utils/command_parser.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\command_parser.py`
- 文件类型：`.py`
- 文件大小：`640` 字节
...
这个文件主要定义了 CommandTokens、CommandParserMixin 等顶层类。
...
- `CommandTokens`：建议阅读类定义与方法名来判断职责。
- `CommandParserMixin`：建议阅读类定义与方法名来判断职责。
