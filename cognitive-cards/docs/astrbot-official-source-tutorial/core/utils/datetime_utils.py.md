# 文件教程：core/utils/datetime_utils.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\datetime_utils.py`
- 文件类型：`.py`
- 文件大小：`820` 字节
...
这个文件主要提供了 normalize_datetime_utc、to_utc_isoformat、to_utc_timestamp 等顶层函数。
...
- `normalize_datetime_utc`：Normalize datetime values to UTC
- `to_utc_isoformat`：建议阅读函数签名和调用位置来判断用途。
- `to_utc_timestamp`：建议阅读函数签名和调用位置来判断用途。
