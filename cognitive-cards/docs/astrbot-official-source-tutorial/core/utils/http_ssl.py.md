# 文件教程：core/utils/http_ssl.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\http_ssl.py`
- 文件类型：`.py`
- 文件大小：`915` 字节
...
这个文件主要提供了 build_ssl_context_with_certifi、build_tls_connector 等顶层函数。
...
- `build_ssl_context_with_certifi`：Build an SSL context from system trust store and add certifi CAs
- `build_tls_connector`：建议阅读函数签名和调用位置来判断用途。
