# 文件教程：core/utils/llm_metadata.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\llm_metadata.py`
- 文件类型：`.py`
- 文件大小：`2355` 字节
...
这个文件主要定义了 LLMModalities、LLMLimit、LLMMetadata 等顶层类。
...
- `LLMModalities`：建议阅读类定义与方法名来判断职责。
- `LLMLimit`：建议阅读类定义与方法名来判断职责。
- `LLMMetadata`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `update_llm_metadata`：建议阅读函数签名和调用位置来判断用途。
