# 文件教程：core/utils/migra_helper.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\migra_helper.py`
- 文件类型：`.py`
- 文件大小：`7173` 字节
...
这个文件主要提供了 _migra_agent_runner_configs、_migra_provider_to_source_structure、migra 等顶层函数。
...
- `_migra_agent_runner_configs`：Migra agent runner configs from provider configs
- `_migra_provider_to_source_structure`：Migrate old provider structure to new provider-source separation
- `migra`：Stores the migration logic here
