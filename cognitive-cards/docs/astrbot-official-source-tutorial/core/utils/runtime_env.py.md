# 文件教程：core/utils/runtime_env.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\runtime_env.py`
- 文件类型：`.py`
- 文件大小：`217` 字节
...
这个文件主要提供了 is_frozen_runtime、is_packaged_desktop_runtime 等顶层函数。
...
- `is_frozen_runtime`：建议阅读函数签名和调用位置来判断用途。
- `is_packaged_desktop_runtime`：建议阅读函数签名和调用位置来判断用途。
