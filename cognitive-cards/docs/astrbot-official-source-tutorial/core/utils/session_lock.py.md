# 文件教程：core/utils/session_lock.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\session_lock.py`
- 文件类型：`.py`
- 文件大小：`1917` 字节
...
这个文件主要定义了 _PerLoopSessionLockManager、SessionLockManager 等顶层类。
...
- `_PerLoopSessionLockManager`：Per-event-loop session lock manager; keeps original simple semantics
- `SessionLockManager`：Thread-safe session lock manager with per-event-loop isolation
