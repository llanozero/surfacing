# 文件教程：core/utils/session_waiter.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\session_waiter.py`
- 文件类型：`.py`
- 文件大小：`7333` 字节
...
- `SessionController`：控制一个 Session 是否已经结束
- `SessionFilter`：如何界定一个会话
- `DefaultSessionFilter`：建议阅读类定义与方法名来判断职责。
- `SessionWaiter`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `session_waiter`：装饰器：自动将函数注册为 SessionWaiter 处理函数，并等待外部输入触发执行
