# 文件教程：core/utils/storage_cleaner.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\storage_cleaner.py`
- 文件类型：`.py`
- 文件大小：`9157` 字节
...
这个文件主要定义了 LogFileConfig、StorageCleaner 等顶层类。
...
- `LogFileConfig`：建议阅读类定义与方法名来判断职责。
- `StorageCleaner`：建议阅读类定义与方法名来判断职责。
