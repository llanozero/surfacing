# 文件教程：core/utils/temp_dir_cleaner.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\temp_dir_cleaner.py`
- 文件类型：`.py`
- 文件大小：`4645` 字节
...
这个文件主要定义了 TempFileInfo、TempDirCleaner 等顶层类。
...
- `TempFileInfo`：建议阅读类定义与方法名来判断职责。
- `TempDirCleaner`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `parse_size_to_bytes`：Parse size in MB to bytes
