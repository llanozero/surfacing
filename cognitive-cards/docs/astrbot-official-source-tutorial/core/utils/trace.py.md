# 文件教程：core/utils/trace.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\trace.py`
- 文件类型：`.py`
- 文件大小：`2238` 字节
...
- `TraceSpan`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `_get_log_broker`：建议阅读函数签名和调用位置来判断用途。
- `_get_trace_logger`：建议阅读函数签名和调用位置来判断用途。
