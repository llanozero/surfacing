# 文件教程：core/utils/webhook_utils.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\utils\webhook_utils.py`
- 文件类型：`.py`
- 文件大小：`2656` 字节
...
这个文件主要提供了 _get_callback_api_base、_get_dashboard_port、_is_dashboard_ssl_enabled、log_webhook_info 等顶层函数。
...
- `_get_callback_api_base`：建议阅读函数签名和调用位置来判断用途。
- `_get_dashboard_port`：建议阅读函数签名和调用位置来判断用途。
- `_is_dashboard_ssl_enabled`：建议阅读函数签名和调用位置来判断用途。
- `log_webhook_info`：打印美观的 webhook 信息日志
- `ensure_platform_webhook_config`：为支持统一 webhook 的平台自动生成 webhook_uuid
