# 文件教程：core/zip_updator.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\core\zip_updator.py`
- 文件类型：`.py`
- 文件大小：`10202` 字节
...
这个文件主要定义了 ReleaseInfo、RepoZipUpdator 等顶层类。
...
- `ReleaseInfo`：建议阅读类定义与方法名来判断职责。
- `RepoZipUpdator`：建议阅读类定义与方法名来判断职责。
