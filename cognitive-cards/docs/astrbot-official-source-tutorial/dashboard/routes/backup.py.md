# 文件教程：dashboard/routes/backup.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\dashboard\routes\backup.py`
- 文件类型：`.py`
- 文件大小：`40972` 字节
...
- `secure_filename`：清洗文件名，移除路径遍历字符和危险字符
- `generate_unique_filename`：生成唯一的文件名，在原文件名后添加时间戳后缀避免重名
