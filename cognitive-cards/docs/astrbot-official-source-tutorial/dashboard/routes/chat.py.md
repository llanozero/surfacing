# 文件教程：dashboard/routes/chat.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\dashboard\routes\chat.py`
- 文件类型：`.py`
- 文件大小：`35783` 字节
...
- `ChatRoute`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `track_conversation`：建议阅读函数签名和调用位置来判断用途。
- `_poll_webchat_stream_result`：建议阅读函数签名和调用位置来判断用途。
