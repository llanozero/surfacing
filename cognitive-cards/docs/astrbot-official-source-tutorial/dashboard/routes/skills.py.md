# 文件教程：dashboard/routes/skills.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\dashboard\routes\skills.py`
- 文件类型：`.py`
- 文件大小：`27767` 字节
...
- `from astrbot.core.computer.computer_client import _discover_bay_credentials, sync_skills_to_active_sandboxes`
...
- `SkillsRoute`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `_to_jsonable`：建议阅读函数签名和调用位置来判断用途。
- `_to_bool`：建议阅读函数签名和调用位置来判断用途。
- `_next_available_temp_path`：建议阅读函数签名和调用位置来判断用途。
