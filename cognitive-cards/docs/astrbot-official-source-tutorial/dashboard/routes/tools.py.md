# 文件教程：dashboard/routes/tools.py

## 文件定位

- 官方路径：`C:\Users\llano\AppData\Local\AstrBot\backend\app\astrbot\dashboard\routes\tools.py`
- 文件类型：`.py`
- 文件大小：`23624` 字节
...
这个文件主要定义了 EmptyMcpServersError、ToolsRoute 等顶层类。
...
- `EmptyMcpServersError`：Raised when mcpServers is empty
- `ToolsRoute`：建议阅读类定义与方法名来判断职责。

## 顶层函数

- `_extract_mcp_server_config`：Extract server configuration from user-submitted mcpServers field
