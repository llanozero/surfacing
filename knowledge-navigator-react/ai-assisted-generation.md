...
- **本地优先**：优先使用本地 LLM（通过 `/api/ai-generate` 接口），不可用时降级为规则摘要算法
...
显示加载态：按钮变为旋转动画
    ...
    │       └─ 失败（超时/无后端）
    │               │
    │               └─ 执行本地降级算法 → 填入 title → Toast: "已生成标题（本地模式）"
    │
    └─ 生成完毕，按钮恢复为 ✨
```
...
| 场景 | 行为 |
|------|------|
| 后端 API 返回 `ok: false` | 降级到本地算法，Toast 提示"已生成（本地模式）" |
| 后端 API 超时（5s） | 同降级策略 |
| 本地降级算法也无法生成 | 按钮恢复可用，不做填充，Toast "生成失败" |
...
- [ ] 生成过程中按钮显示加载态（不可点击）
- [ ] 后端 API 不可用时降级为本地算法，Toast 标注"本地模式"
