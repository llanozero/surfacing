...
export const run: CommandModule['run'] = async (api, args, flags) => {
  ...
  switch (sub) {
    case 'start': {
      ...
      if (planId) {
        if (succeed(api.initBrowseFromPlan(planId))) okMsg(`已从计划 ${planId} 开始浏览`)
        return
      }
      if (sequence) {
        const ids = sequence.split(',').map((s) => s.trim()).filter(Boolean)
        if (succeed(api.initBrowseFromSequence(ids))) okMsg(`已从节点序列开始浏览（${ids.length} 站）`)
        return
      }
      // 默认：当前选中的计划
      const selected = api.getSelectedPlan()
      if (!selected) return errMsg('请先选中一个计划（plan select / --plan / --sequence）')
      if (succeed(api.initBrowseFromPlan(selected.id))) okMsg(`已从当前计划 ${selected.id} 开始浏览`)
      return
    }
    case 'status': {
      const progress = api.getBrowseProgress()
    ...
    case 'cards': {
      const cards = api.getCurrentBrowseCards()
      if (json) return printJson(cards)
      printLines(cards.map((c, i) => `[${i}] ${c.title}${c.tag ? `  #${c.tag}` : ''}  ${c.desc.slice(0, 40)}`), '（无浏览卡片，请先 browse start）')
      return
    }
    case 'next': {
      if (succeed(api.nextCard())) okMsg('已切换到下一张卡片')
      return
    }
    case 'prev': {
      if (succeed(api.prevCard())) okMsg('已切换到上一张卡片')
      return
    }
    case 'waypoint': {
      if (succeed(api.nextWaypoint())) okMsg('已切换到下一站')
      return
    }
    default:
