...
export const run: CommandModule['run'] = async (api, args, flags) => {
  ...
  switch (sub) {
    ...
    case 'search': {
      ...
      const matched = api.getTreeFlatData().filter((n) => n.title.toLowerCase().includes(q))
