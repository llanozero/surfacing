...
target_node_id: node-supervised
route_plan:
  nodes:
    - node-probability
    - node-ml-foundation
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `current_node_id` | string | 当前所在导航节点 id。 |
| `search_query` | string | 用户输入的搜索文本。 |
| `matched_card_id` | string | 搜索匹配到的认知卡片 id。 |
| `target_node_id` | string | 用户选定的目标导航节点 id。 |
| `route_plan` | object | 导航路线规划结果，包含节点序列和总体权重（原 `path_plan`，已去除 `path` 命名）。 |
| `browse_index` | number | 内容浏览视图中的当前卡片序号。 |
...
│  → 找到绑定的导航节点(card.bound_nodes)                         │
│  → 高亮节点 → 点击跳转                                         │
│  → 传递: { matched_card_id, node_id }                          │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  展示当前节点 + 前置节点 + 候选节点                                │
│  搜索/输入 → 匹配认知卡片 → 选择目标导航节点 → 建立新连接          │
│  上拉面板: 编辑节点信息、配置权重、建新连接                        │
│  传递: { current_node_id, target_node_id, route_plan }          │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                      内容浏览 (Browse View)                      │
│  card_id → 加载卡片详情 → 上下滑动切换                           │
│  模式切换: 上滑/下滑为下一节点                                   │
└─────────────────────────────────────────────────────────────────┘
